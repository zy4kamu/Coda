...
static char * env_scan(register char *str)
{
    extern char *getenv();
...
static int SetVal (register Config_t *cp, char *str)
{
    switch (cp->arg_type) {
    case CHAR:
...
static void SPrintVal (register Config_t *cp, char *str)
{
    switch (cp->arg_type) {
    case CHAR:
 ...
 */
extern
int fpconf (FILE *config_fp, Config_t config_p[], char **display, char **geometry, char *(*GetDefault)())
{
	...
	for (parsed = FALSE, cp = config_p; cp->arg_type != NOTYPE; cp++) {
	    if (strcmp (name, cp->LongName) == 0) { // TODO strcasecmp
		parsed = TRUE;
