...
static int find_net(char *name, Gram *gram);
...
void read_nets(char *net_file, Gram *gram, char ** sb_start, char * sym_buf_end)
{
	...
	if( sscanf(rul_ptr, "Number of Nets= %d", &num_nets) < 1 ) {
	...
	/* malloc space for net features pointers */
	if( !(gram->feats= (char **) calloc(gram->num_nets, sizeof(char *))) ) {
	fprintf(stderr, "ERROR: malloc for features failed\n");
	exit(-1);
	}
	...
	while( fscanf(fp, "%s %d %d %d %s", name, &net_num, &num_nodes, &val, feats) == 5 ) {
	...
	gram->node_counts[net_num]= num_nodes;
		...
		/* copy net feats to feats array */
		strcpy(sym_ptr, feats);
		gram->feats[net_num]= sym_ptr;
		sym_ptr += strlen(sym_ptr) +1;
		if( sym_ptr >= sym_buf_end ) {
		fprintf(stderr, "ERROR: overflow SymBufSize %d\n", SymBufSize);
		exit(-1);
	...
	gram->Nets[net_num]= net;
		...
		if( fscanf(fp, "%d %d %d %d %s", &succ->tok,
				&succ->call_net, &offset, &succ->isHead, &succ->feats) != 5) {
...
void read_frames(char *dir, char * frames_file,  Gram *gram, char **sb_start, char ** sym_buf_end)
{
	...
	while( (r= fgets(line, LINE_LEN, fp)) ) {
			sscanf(line, "%s%*[^\n]\n", name);
		if( !strncmp(name, "FUNCTION:", 6) ) break;
	}
	if( !r ) break;
